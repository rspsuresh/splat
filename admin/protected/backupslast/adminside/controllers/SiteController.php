...
class SiteController extends Controller
{
    ...
    public function actionAssessment($id)
    {
        ...
        /*if(isset($_POST['comments'])){
            foreach($_POST['comments'] as $key=>$ucomments){
                if(trim($ucomments)!='') {
                    $comments = AssessComments::model()->find('project=:p and from_user=:f and to_user=:t',array(':p'=>$id,':f'=>Yii::app()->user->id,':t'=>$key));
                    if(count($comments)>0){
                        $comments->asses_id=$_POST['assesmentid'];
                        $comments->comments = $ucomments;
                        $comments->save();
                    } else{
                        $comments = new AssessComments();
                        $comments->asses_id=$_POST['assesmentid'];
                        $comments->project = $id;
                        $comments->from_user = Yii::app()->user->id;
                        $comments->to_user = $key;
                        $comments->comments = $ucomments;
                        $comments->save();
                    }
                }
